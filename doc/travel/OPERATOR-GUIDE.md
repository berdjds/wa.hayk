...
2. Build the itinerary and scenarios on the request page (Scenarios tab). Adding a hotel with
   its own dates splits the existing stay (e.g. inserting 3–5 Oct into 1–6 Oct yields
   2+2+1 nights — previewed before saving). Use "Calculate preview" to see per-scenario
   costs, issues and selling totals; alternative scenarios are never summed together.
