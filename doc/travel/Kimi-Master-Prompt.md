...
- Absolute minimum package profit in a specified currency: `floor = cost + minProfit` after consistent conversion. **(v0.15.1: per traveler excluding infants — `floor = cost + minProfit × max(0, adults + children − infants)`; supersedes the v0.14.0 per-paying-person multiplier.)**
