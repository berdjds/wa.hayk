...
- Minimum profit: `floorSell = cost + minimumProfit`, in the configured policy currency converted using the frozen FX. **(Superseded in v0.14.0: per paying person; superseded again in v0.15.1: the floor is per traveler excluding infants — `floorSell = cost + minimumProfit × max(0, adults + children − infants)`. Snapshots frozen before v0.15.1 keep their numbers.)**
